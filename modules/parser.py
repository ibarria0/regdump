...
def collect_nombre_sociedad(soup):
  return sanitize(soup.find(text="Nombre de la Sociedad:").parent.parent.parent.parent.find_next('td').string)

def collect_nombre_fundacion(soup):
...
def collect_documento(soup):
  return int(soup.find(text='No. Documento:').parent.parent.parent.find_next_sibling().p.string)

def collect_escritura(soup):
  return int(soup.find(text='No. Escritura:').parent.parent.parent.find_next_sibling().string)
